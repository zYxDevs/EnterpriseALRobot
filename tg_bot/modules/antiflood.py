...
from tg_bot import SARDEGNA_USERS, WHITELIST_USERS, app as application
...
import contextlib
@kigmsg(
    (filters.ALL & ~filters.StatusUpdate.ALL & filters.ChatType.GROUPS), group=FLOOD_GROUP
)
@connection_status
@loggable
async def check_flood(update, context) -> Optional[str]:
    global execstrings
...
@can_restrict
@loggable
async def set_flood(update, context) -> str:  # sourcery no-metrics
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
    ...
    if len(args) >= 1:
        ...
        if val in ["off", "no", "0"]:
            ...
            if amount <= 0:
                ...
                if conn:
                    text = await message.reply_text(f"Anti-flood has been set to {amount} in chat: {chat_name}")

                else:
                    text = await message.reply_text(f"Successfully updated anti-flood limit to {amount}!")
...
async def flood(update, context):
    ...
    if limit == 0:
        return msg.reply_text(f"I'm not enforcing any flood control in {chat_name}!") if conn else msg.reply_text("I'm not enforcing any flood control here!")

    elif conn:
        return msg.reply_text(f"I'm currently restricting members after {limit} consecutive messages in {chat_name}.")


    else:
        return msg.reply_text(f"I'm currently restricting members after {limit} consecutive messages.")


@kigcmd(command="setfloodmode", filters=filters.ChatType.GROUPS)
@user_admin(AdminPerms.CAN_CHANGE_INFO)
async def set_flood_mode(update, context):  # sourcery no-metrics
    global settypeflood
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
    msg = update.effective_message  # type: Optional[Message]
...
Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks.""",
            ...
            return
        if conn:
            text = msg.reply_text(f"Exceeding consecutive flood limit will result in {settypeflood} in {chat_name}!")

        else:
            text = msg.reply_text(f"Exceeding consecutive flood limit will result in {settypeflood}!")
        ...
        if conn:
            text = msg.reply_text(f"Sending more messages than flood limit will result in {settypeflood} in {chat_name}.")

        else:
            text = msg.reply_text(f"Sending more message than flood limit will result in {settypeflood}.")
